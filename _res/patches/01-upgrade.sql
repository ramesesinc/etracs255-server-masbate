...
delete from sys_usergroup_member where usergroup_objid like 'ACCOUNTING%';
...
; 
delete from cashreceiptitem where receiptid not in (
  select objid from cashreceipt 
  where objid = cashreceiptitem.receiptid 
) 
;
...
alter table cashreceiptpayment_noncash drop foreign key cashreceiptpayment_noncash_ibfk_1;
...
create table ztmp_ei 
select ei.objid, 
  concat(ifnull(ei.firstname,''),' ',ifnull(ei.middlename,''),' ',ifnull(ei.lastname,'')) as fullname 
from entityindividual ei 
where LENGTH(ei.firstname) > 100 
; 
insert into entitymultiple ( objid, fullname )
select objid, fullname 
from ztmp_ei ei 
where objid not in (select objid from entitymultiple where objid = ei.objid) 
; 
delete from entityindividual where objid in (
  select objid from ztmp_ei 
)
;
drop table ztmp_ei
;
...
alter table creditmemoitem drop foreign key creditmemoitem_ibfk_2;
...
update depositvoucher set state = 'POSTED' where controldate < '2021-04-01' and state = 'OPEN'
;
